    ...
    try:
        ...
        if has_alpha:
            ...
            if len(split_bands) == 4:
                ...
                # Закриваємо канали RGB з split_bands, вони більше не потрібні
...
def add_padding(img, percent):
    ...
    if percent <= 0:
        return img
    # Не виводимо повідомлення тут, виводитимемо в process_image
    ...
    try:
        ...
        nw, nh = w + 2*pp, h + 2*pp
        # print(f"      - Оригінал: {w}x{h}, Поле: {pp}px, Новий розмір: {nw}x{nh}") # Повідомлення в process_image
        ...
        # print("      - Поля успішно додано.") # Повідомлення в process_image
...
# <<< МОДИФІКАЦІЯ: Додано padding_percent, перенесено логіку полів сюди >>>
def process_image(image_path, preprocess_max_width, preprocess_max_height,
                  enable_whitening, whitening_cancel_threshold_sum,
                  white_tolerance, perimeter_margin, padding_percent, # <<< Додано padding_percent
    ...
    pre_crop_width, pre_crop_height = 0, 0 # <<< Розміри до обрізки/видалення фону
    cropped_image_dimensions = None        # <<< Розміри після обрізки (для логування полів)
    ...
    try:
        ...
        if processed_img:
            pre_crop_width, pre_crop_height = processed_img.size
            print(f"  - Розмір перед видаленням фону/обрізкою: {pre_crop_width}x{pre_crop_height}")
        else: # Малоймовірно тут, але про всяк випадок
            print("  !! ПОМИЛКА: Немає зображення перед кроком видалення фону/обрізки.")
            return None # Не можемо продовжити
        ...
        if enable_bg_removal_and_crop:
            ...
            # Зберігаємо розмір після обрізки для логування полів
            if processed_img:
                 cropped_image_dimensions = processed_img.size
            ...
            if not processed_img or processed_img.size[0] <= 0 or processed_img.size[1] <= 0:
             ...
             # Розмір "після обрізки" = розміру "до обрізки"
             if processed_img:
                 cropped_image_dimensions = processed_img.size

        # --- 7. Додавання полів (Опціонально, з перевіркою розміру) ---
        # <<< НОВА ЛОГІКА >>>
        apply_padding_final = False
        if padding_percent is not None and padding_percent > 0:
             print(f"  - Крок 7: Додавання полів ({padding_percent}%)...")
             ...
             if current_w > 0 and current_h > 0 and (pre_crop_width > 0 and pre_crop_height > 0):
                 ...
                 if padding_pixels > 0:
                     ...
                     if cropped_image_dimensions: # Використовуємо збережений розмір
                         print(f"      - Після обрізки:     {cropped_image_dimensions[0]}x{cropped_image_dimensions[1]}")
                     else: # Якщо кроку обрізки не було
                 ...
                 print(f"    - Поля не додаються (поточний розмір {current_w}x{current_h}, розмір до обрізки {pre_crop_width}x{pre_crop_height}).")
                 apply_padding_final = False
             # --- Кінець перевірки розміру ---

             # Виконуємо додавання полів тільки якщо apply_padding_final = True
             if apply_padding_final:
                 padded = add_padding(processed_img, padding_percent)
                 if padded is not processed_img:
                      safe_close(processed_img)
                      processed_img = padded
                      print(f"    - Поля додано. Новий розмір: {processed_img.size}, Режим: {processed_img.mode}")
                 else:
                      print(f"    - Додавання полів не змінило зображення.")
             # else: Повідомлення про пропуск вже було вище

        else:
             print("  - Крок 7: Додавання полів вимкнено або відсоток = 0.")
        ...
        if processed_img.mode != 'RGBA':
            ...
            print(f"    - Фінальний режим: {processed_img.mode} (OK)")
...
# --- Функція об'єднання ---
# <<< МОДИФІКАЦІЯ: Прибрано крок 3 (Padding), оновлено виклик process_image >>>
def combine_images(image_paths, output_path,
                   preprocess_max_width, preprocess_max_height,
                   enable_whitening, whitening_cancel_threshold_sum,
                   forced_cols, spacing_percent, white_tolerance, quality,
                   perimeter_margin, padding_percent, # <<< Параметр все ще потрібен для process_image
                   crop_symmetric_axes, crop_symmetric_absolute,
    ...
    processed_images = [] # Для результатів process_image (вже можуть мати поля)
    ...
    try:
        ...
        for i, path in enumerate(image_paths):
            # Перевірка, щоб не обробляти сам вихідний файл
            ...
            # <<< МОДИФІКАЦІЯ: Передаємо padding_percent в process_image >>>
            processed = process_image(
                ...
                padding_percent=padding_percent, # <<< Передаємо сюди
        ...
        # Тепер працює з processed_images, результат в scaled_images
        ...
        # (Логіка перенесена в process_image)

        # Перевірка після перших кроків
        num_final_images = len(scaled_images) # Тепер перевіряємо scaled_images
        if num_final_images == 0:
            print("\nНемає зображень для об'єднання після кроків обробки/масштабування.")
            return

        # --- 4. Розрахунок розмірів колажу ---
        # Тепер базується на scaled_images
        ...
        # Знаходимо максимальні розміри комірки (на основі scaled_images)
        ...
        h_spacing = int(round(max_w * (spacing_percent / 100.0)))
        v_spacing = int(round(max_h * (spacing_percent / 100.0)))
        ...
        # Тепер розміщуємо з scaled_images
        ...
        for r in range(grid_rows):
            ...
            for c in range(items_in_row):
                ...
                img = scaled_images[current_idx] # <<< Беремо з scaled_images
                if img and img.size[0] > 0 and img.size[1] > 0:
                     px = start_x_offset + h_spacing + c * (max_w + h_spacing)
        ...
        # Закриваємо зображення з scaled_images
        ...
        # (Нумерація кроків у логах змінюється)
        print("\n--- КРОК 4 (Раніше 5): Трансформації колажу (Співвідношення, Макс.розмір, Точний холст) ---")
        # (Код цього кроку залишається без змін, працює з collage_step_img)
            ...
            if cw > 0 and ch > 0: # Перевірка на нульовий розмір
                if cw != tw or ch != th:
                    ...
                    try:
                        ...
                        if temp_content.mode == 'RGBA': paste_mask = temp_content # Використовуємо альфа-канал RGBA як маску
                        ...
                        print(f"    - Новий розмір (точний холст): {collage_step_img.size}, Режим: {collage_step_img.mode}")
                         ...
                         safe_close(temp_content); safe_close(temp_canvas)
                    finally:
                         safe_close(temp_content) # Закриваємо масштабований вміст
                         if temp_canvas: safe_close(temp_canvas) # temp_canvas закриється, якщо не став collage_step_img
                else:
        ...
        if final_img_to_save.size[0] <= 0 or final_img_to_save.size[1] <= 0:
             print(f"!! ПОМИЛКА: Фінальне зображення має нульовий розмір {final_img_to_save.size}."); safe_close(final_img_to_save); return
        ...
        safe_close(temp_img)
        # Закриваємо списки зображень
        for lst in [processed_images, scaled_images]: # <<< Прибрано padded_images
...
# <<< МОДИФІКАЦІЯ: Незначні зміни у виводі та санітизації, передача padding_percent >>>
def run_processing(source_dir, output_filename,
                   preprocess_max_width, preprocess_max_height,
                   enable_whitening, whitening_cancel_threshold_sum,
                   forced_cols, perimeter_margin, padding_percent, # <<< padding_percent ТУТ
                   white_tolerance,
    ...
    # <<< Змінено опис полів >>>
    print(f"  - Додавання полів: {p_perc_print}% (умовно, якщо обрізка була суттєвою)")
    ...
    # (Решта виводу налаштувань колажу без змін)
    ...
    # (Вивід налаштувань збереження без змін)
    ...
    if not input_files_found: print(f"\n!!! У папці '{source_dir}' не знайдено файлів з розширеннями: {SUPPORTED_EXTENSIONS}"); return

    # Сортування знайдених файлів
    try: input_files_sorted = natsorted(input_files_found)
    except Exception as sort_err: print(f"! Помилка натурального сортування: {sort_err}. Використання стандартного сортування."); input_files_sorted = sorted(input_files_found)
    ...
    ratio_tuple = None
    if ratio_valid: # Використовуємо прапор, встановлений при друку
    ...
    # <<< МОДИФІКАЦІЯ: Передаємо санітизований p_percent >>>
    combine_images(
        ...
        padding_percent=p_percent, # <<< Передаємо санітизований відсоток
        crop_symmetric_axes=crop_symmetric_axes,
        crop_symmetric_absolute=crop_symmetric_absolute,
...
if __name__ == "__main__":
    ...
    output_filename = "collage_final_v2.jpg" # !!! Змініть, якщо потрібно
    ...
    # Поля додаються ПІСЛЯ обрізки, АЛЕ ТІЛЬКИ ЯКЩО розмір з полями <= розмір ДО обрізки
    ...
    run_processing(
        ...
        padding_percent=padding_percent, # <<< Передаємо відсоток сюди
        white_tolerance=white_tolerance,
