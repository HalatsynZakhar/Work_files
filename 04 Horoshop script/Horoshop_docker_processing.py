...
def extract_data_from_docker(container_name):
    """Извлекает и парсит файл url-watches.json напрямую из Docker контейнера"""
...
def extract_article_info(url, title=""):
    ...
    # Если в url не было html, пробуем взять первое слово из названия
    art_from_title = ""
    if title:
    ...
    try:
        ...
        for ws in wb.worksheets:
            for col in ws.columns:
                ...
                # Ограничиваем ширину ссылок разумным значением
...
def main():
    ...
    # Оставляем исходные колонки Excel + Цена
    ...
    # 9. Сохранение файла ИТОГОВЫЙ_ОТЧЕТ.xlsx (содержит все листы для удобства)
    try:
        ...
        auto_fit_excel_columns(path_output)
        ...
        print(f"  Лист 3: 'Полная информация'         ({len(sheet3_detailed)} позиций, с ссылками и магазинами)")
        print(f"  Лист 4: 'База трекера (все товары)' ({len(df_full_tracker)} позиций из Docker)")
    except Exception as e:
        print(f"\n[ОШИБКА] Ошибка при сохранении ИТОГОВЫЙ_ОТЧЕТ.xlsx: {e}")

    # 10. Сохранение отдельного файла ПОЛНАЯ_ТАБЛИЦА_МОНИТОРИНГА.xlsx
    try:
        with pd.ExcelWriter(path_full_output, engine='openpyxl') as writer:
        ...
        auto_fit_excel_columns(path_full_output)

        print("-" * 65)
        print("   СОЗДАНА ДОПОЛНИТЕЛЬНАЯ ПОЛНАЯ ТАБЛИЦА!   ")
        print("-" * 65)
        print(f"Файл: {path_full_output}")
        ...
        print(f"\n[ОШИБКА] Ошибка при сохранении ПОЛНАЯ_ТАБЛИЦА_МОНИТОРИНГА.xlsx: {e}")
